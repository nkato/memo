...
def draw_calibration_curve(
    ...
    """
    in_range_idx = np.where((draw_range[0] <= y_pred) & (y_pred <= draw_range[1]))
    ...
    y_pos_pred_in_range = [p for t, p in zip(y_true_in_range, y_pred_in_range) if t == 1]
    y_neg_pred_in_range = [p for t, p in zip(y_true_in_range, y_pred_in_range) if t == 0]

    ax1 = fig.add_subplot(spec[1])
    ax1.hist(y_neg_pred_in_range, range=draw_range, bins=n_bins, alpha=0.5, density=True, label="Negative")
    ax1.hist(y_pos_pred_in_range, range=draw_range, bins=n_bins, alpha=0.5, density=True, label="Positive")
    ax1.set(xlim=draw_range, xlabel="Mean predicted probability", ylabel="Density")
